...
const catIcon = { Clothing:'👕', Footwear:'👟', Electronics:'🔌', Toiletries:'🧴', Documents:'📄', 'Health & Safety':'💊', 'Activity Gear':'🎒' };

const PackingItem = ({ item, isChecked, isInTrip, onToggleCheck, onAddToTrip, onRemoveFromTrip }) => (
  <div className={`${styles.row} ${isChecked ? styles.checked : ''}`}>
    ...
    {isInTrip ? (
      <button className={styles.removeBtn} onClick={() => onRemoveFromTrip(item._id)} aria-label="remove">✕</button>
    ) : (
      <button className={styles.addBtn} onClick={() => onAddToTrip(item)}>+ Add</button>
    )}
  </div>
);
...
PackingItem.defaultProps = {
  isChecked: false, isInTrip: false,
  onToggleCheck: () => {}, onAddToTrip: () => {}, onRemoveFromTrip: () => {},
};
